...
RUN curl -LfsS https://mirrors.tuna.tsinghua.edu.cn/gnu/glibc/glibc-${GLIBC_VERSION}.tar.xz | tar xfJ - && \
...
RUN /glibc-${GLIBC_VERSION}/configure \
    ...
    make && make install
RUN	tar --dereference --hard-dereference -zcf /glibc-bin-${GLIBC_VERSION}.tar.gz ${PREFIX_DIR}
...
COPY --from=glibc-builder /glibc-bin-${GLIBC_VERSION}.tar.gz /home/builder/upstream
